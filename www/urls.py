...
def make_signed_cookie(id, password, max_age):
    ...
    L = [id, expires, hashlib.md5(('%s-%s-%s-%s' % (id, password, expires, _COOKIE_KEY)).encode('utf-8')).hexdigest()]
    return '-'.join(L)
...
def register_user():
    ...
    user = User(name=name,email=email,password=password,image='http://www.gravatar.com/avatar/%s?d=mm&s=120' % hashlib.md5(email.encode('utf-8')).hexdigest())
    user.insert()
